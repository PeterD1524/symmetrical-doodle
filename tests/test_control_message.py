...
def test_serialize_inject_keycode():
    message = symmetrical_doodle.control_message.InjectKeycode(
        action=symmetrical_doodle.android.input.KeyEventAction.AKEY_EVENT_ACTION_UP,
        keycode=symmetrical_doodle.android.keycodes.Keycode.AKEYCODE_ENTER,
        repeat=5,
        meta_state=symmetrical_doodle.android.input.MetaState.AMETA_SHIFT_ON.value
...
def test_serialize_inject_text():
    message = symmetrical_doodle.control_message.InjectText(text=b"hello, world!")
    buf = message.serialize()
...
def test_serialize_inject_touch_event():
    message = symmetrical_doodle.control_message.InjectTouchEvent(
        action=symmetrical_doodle.android.input.MotionEventAction.AMOTION_EVENT_ACTION_DOWN,
        ...
        position=symmetrical_doodle.coords.Position(
            screen_size=symmetrical_doodle.coords.Size(width=1080, height=1920),
    ...
    assert (
        buf
        == b"\x02\x00\x124Vx\x87eC!\x00\x00\x00d\x00\x00\x00\xc8\x048\x07\x80\xff\xff\x00\x00\x00\x01"
    )
...
def test_serialize_inject_scroll_event():
    ...
    assert (
        buf
        == b"\x03\x00\x00\x01\x04\x00\x00\x04\x02\x048\x07\x80\x00\x00\x00\x01\xff\xff\xff\xff\x00\x00\x00\x01"
    )
...
def test_serialize_set_clipboard_long():
    text_length = (
        symmetrical_doodle.control_message.CONTROL_MSG_CLIPBOARD_TEXT_MAX_LENGTH
    )
    text = b"a" * text_length
    message = symmetrical_doodle.control_message.SetClipboard(
    ...
    buf = message.serialize()
    expected = (
        b"\x09\x01\x02\x03\x04\x05\x06\x07\x08\x01\x00\x03\xff\xf2" + b"a" * text_length
    )
    assert buf == expected
