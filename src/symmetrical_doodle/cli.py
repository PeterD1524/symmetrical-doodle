...
def get_parser():
    ...
    parser.add_argument(
        "--codec-options",
        help="Set a list of comma-separated key:type=value options for the device encoder. The possible values for 'type' are 'int' (default), 'long', 'float' and 'string'. The list of possible codec options is available in the Android documentation: <https://d.android.com/reference/android/media/MediaFormat>",
        metavar="key[:type]=value[,...]",
    )
    parser.add_argument(
        "--crop",
        help="Crop the device screen on the server. The values are expressed in the device natural orientation (typically, portrait for a phone, landscape for a tablet). Any --max-size value is computed on the cropped size.",
    ...
    parser.add_argument(
        ...
        "--select-usb",
        action="store_true",
        help="Use USB device (if there is exactly one, like adb -d). Also see -e (--select-tcpip).",
    )
    parser.add_argument(
        "--disable-screensaver",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        help='Specify the display id to mirror. The list of possible display ids can be listed by: adb shell dumpsys display (search "mDisplayId=" in the output). Default is %(default)s.',
        metavar="id",
    )
    parser.add_argument(
        "--display-buffer",
        ...
        type=int,
        help="Add a buffering delay (in milliseconds) before displaying. This increases latency to compensate for jitter. Default is %(default)s (no buffering).",
        metavar="ms",
    )
    parser.add_argument(
        "-e",
        "--select-tcpip",
        action="store_true",
        help="Use TCP/IP device (if there is exactly one, like adb -e). Also see -d (--select-usb).",
    )
    parser.add_argument(
        "--encoder",
    ...
    parser.add_argument(
        "--forward-all-clicks",
        action="store_true",
        help="By default, right-click triggers BACK (or POWER on) and middle-click triggers HOME. This option disables these shortcuts and forwards the clicks to the device instead.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--hid-keyboard",
        action="store_true",
        help="Simulate a physical keyboard by using HID over AOAv2. It provides a better experience for IME users, and allows to generate non-ASCII characters, contrary to the default injection method. It may only work over USB. The keyboard layout must be configured (once and for all) on the device, via Settings -> System -> Languages and input -> Physical keyboard. This settings page can be started directly: `adb shell am start -a android.settings.HARD_KEYBOARD_SETTINGS`. However, the option is only available when the HID keyboard is enabled (or a physical keyboard is connected). Also see --hid-mouse.",
    )
    parser.add_argument(
        "--legacy-paste",
        action="store_true",
        help="Inject computer clipboard text as a sequence of key events on Ctrl+v (like MOD+Shift+v). This is a workaround for some devices not behaving as expected when setting the device clipboard programmatically.",
    )
    parser.add_argument(
        "--lock-video-orientation",
        nargs="?",
        default=options_default.lock_video_orientation.name.lower(),
        help='Lock video orientation to value. Possible values are "unlocked", "initial" (locked to the initial orientation), 0, 1, 2 and 3. Natural device orientation is 0, and each increment adds a 90 degrees rotation counterclockwise. Default is "%(default)s". Passing the option without argument is equivalent to passing "initial".',
        metavar="value",
    )
    parser.add_argument(
        "--max-fps",
        ...
        type=int,
        help="Limit the frame rate of screen capture (officially supported since Android 10, but may work on earlier versions).",
        metavar="value",
    )
    parser.add_argument(
        "-M",
        "--hid-mouse",
        action="store_true",
        help="Simulate a physical mouse by using HID over AOAv2. In this mode, the computer mouse is captured to control the device directly (relative mouse mode). LAlt, LSuper or RSuper toggle the capture mode, to give control of the mouse back to the computer. It may only work over USB. Also see --hid-keyboard.",
    )
    parser.add_argument(
        "-m",
        "--max-size",
        ...
        type=int,
        help="Limit both the width and height of the video to value. The other dimension is computed so that the device aspect-ratio is preserved. Default is %(default)s (unlimited).",
        metavar="value",
    )
    parser.add_argument(
        "--no-cleanup",
        action="store_true",
        help="By default, scrcpy removes the server binary from the device and restores the device state (show touches, stay awake and power mode) on exit. This option disables this cleanup.",
    )
    parser.add_argument(
        "--no-clipboard-autosync",
        action="store_true",
        help="By default, scrcpy automatically synchronizes the computer clipboard to the device clipboard before injecting Ctrl+v, and the device clipboard to the computer clipboard whenever it changes. This option disables this automatic synchronization.",
    )
    parser.add_argument(
        "--no-downsize-on-error",
        action="store_true",
        help="By default, on MediaCodec error, scrcpy automatically tries again with a lower definition. This option disables this behavior.",
    )
    parser.add_argument(
        "-n",
        "--no-control",
    ...
    parser.add_argument(
        ...
        "--no-display",
        action="store_true",
        help="Do not display device (only when screen recording or V4L2 sink is enabled).",
    )
    parser.add_argument(
        "--no-key-repeat",
        action="store_true",
    ...
    parser.add_argument(
        "--no-mipmaps",
        action="store_true",
        help="If the renderer is OpenGL 3.0+ or OpenGL ES 2.0+, then mipmaps are automatically generated to improve downscaling quality. This option disables the generation of mipmaps.",
    )
    parser.add_argument(
        "--otg",
        action="store_true",
        help="Run in OTG mode: simulate physical keyboard and mouse, as if the computer keyboard and mouse were plugged directly to the device via an OTG cable. In this mode, adb (USB debugging) is not necessary, and mirroring is disabled. LAlt, LSuper or RSuper toggle the mouse capture mode, to give control of the mouse back to the computer. If any of --hid-keyboard or --hid-mouse is set, only enable keyboard or mouse respectively, otherwise enable both. It may only work over USB. See --hid-keyboard and --hid-mouse.",
    )
    parser.add_argument(
        "-p",
        "--port",
        type=int,
        help="Set the TCP port (range) used by the client to listen. Default is %(default)s.",
        metavar="port",
    )
    parser.add_argument(
        "--power-off-on-close",
    ...
    parser.add_argument(
        "--prefer-text",
        action="store_true",
        help="Inject alpha characters and space as text events instead of key events. This avoids issues when combining multiple keys to enter a special character, but breaks the expected behavior of alpha keys in games (typically WASD).",
    )
    parser.add_argument(
        "--print-fps",
        action="store_true",
        help="Start FPS counter, to print framerate logs to the console. It can be started or stopped at any time with MOD+i.",
    )
    parser.add_argument(
        "--push-target",
        default=options_default.push_target,
        help='Set the target directory for pushing files to the device by drag & drop. It is passed as is to "adb push". Default is "%(default)s".',
        metavar="path",
    )
    parser.add_argument(
        "--raw-key-events",
    ...
    parser.add_argument(
        "-r",
        "--record",
        help="Record screen to file. The format is determined by the --record-format option if set, or by the file extension (.mp4 or .mkv).",
        metavar="file.mp4",
    )
    parser.add_argument(
        "--record-format",
    ...
    parser.add_argument(
        "--render-driver",
        help='Request SDL to use the given render driver (this is just a hint). Supported names are currently "direct3d", "opengl", "opengles2", "opengles", "metal" and "software". <https://wiki.libsdl.org/SDL_HINT_RENDER_DRIVER>',
        metavar="name",
    )
    parser.add_argument(
        "--rotation",
        choices=range(4),
        help="Set the initial display rotation. Possible values are 0, 1, 2 and 3. Each increment adds a 90 degrees rotation counterclockwise.",
        metavar="value",
    )
    parser.add_argument(
        "-s",
        "--serial",
        help="The device serial number. Mandatory only if several devices are connected to adb.",
        metavar="serial",
    )
    parser.add_argument(
        "--shortcut-mod",
    ...
    parser.add_argument(
        ...
        "--show-touches",
        action="store_true",
        help='Enable "show touches" on start, restore the initial value on exit. It only shows physical touches (not clicks from scrcpy).',
    )
    parser.add_argument(
        "--tcpip",
    ...
    parser.add_argument(
        "--tunnel-host",
        help="Set the IP address of the adb tunnel to reach the scrcpy server. This option automatically enables --force-adb-forward. Default is localhost.",
        metavar="ip",
    )
    parser.add_argument(
        "--tunnel-port",
        ...
        type=int,
        help="Set the TCP port of the adb tunnel to reach the scrcpy server. This option automatically enables --force-adb-forward. Default is %(default)s (not forced): the local port used for establishing the tunnel will be used.",
    )
    parser.add_argument(
        "--v4l2-sink",
        metavar="/dev/videoN",
        help="Output to v4l2loopback device. It requires to lock the video orientation (see --lock-video-orientation). This feature is only available on Linux.",
    )
    parser.add_argument(
        "--v4l2-buffer",
        default=options_default.v4l2_buffer,
        type=int,
        help="Add a buffering delay (in milliseconds) before pushing frames. This increases latency to compensate for jitter. This option is similar to --display-buffer, but specific to V4L2 sink. Default is %(default)s (no buffering). This option is only available on Linux.",
        metavar="ms",
    )
    parser.add_argument(
        "-V",
        ...
        help="Set the log level (verbose, debug, info, warn or error). Default is %(default)s.",
        metavar="value",
    )
    parser.add_argument(
        "-v",
    ...
    parser.add_argument(
        ...
        "--stay-awake",
        action="store_true",
        help="Keep the device on while scrcpy is running, when the device is plugged in.",
    )
    parser.add_argument(
        "--window-borderless",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help='Set the initial window horizontal position. Default is "%(default)s".',
        metavar="value",
    )
    ...
    parser.add_argument(
        ...
        help='Set the initial window vertical position. Default is "%(default)s".',
        metavar="value",
    )
    parser.add_argument(
        "--window-width",
        ...
        type=int,
        help="Set the initial window width. Default is %(default)s (automatic).",
        metavar="value",
    )
    parser.add_argument(
        "--window-height",
        ...
        type=int,
        help="Set the initial window height. Default is %(default)s (automatic).",
        metavar="value",
    )
...
def parse_args(args=None):
    ...
    if args.hid_keyboard:
        kwargs["keyboard_input_mode"] = symmetrical_doodle.options.KeyboardInputMode.HID
    ...
    if args.hid_mouse:
        kwargs["mouse_input_mode"] = symmetrical_doodle.options.MouseInputMode.HID

    lock_video_orientation = parse_lock_video_orientation(args.lock_video_orientation)

    log_level = parse_log_level(args.verbosity)

    window_x = parse_window_position(args.window_x)
    assert (
        -0x7FFF <= window_x <= 0x7FFF
        or window_x == symmetrical_doodle.options.WINDOW_POSITION_UNDEFINED
    )

    window_y = parse_window_position(args.window_y)
    assert (
        -0x7FFF <= window_y <= 0x7FFF
        or window_y == symmetrical_doodle.options.WINDOW_POSITION_UNDEFINED
    )
    ...
    if prefer_text:
        kwargs["key_inject_mode"] = symmetrical_doodle.options.KeyInjectMode.TEXT
    if raw_key_events:
        kwargs["key_inject_mode"] = symmetrical_doodle.options.KeyInjectMode.RAW
    ...
    serial = args.serial
    assert (
        sum([serial is not None, tcpip_dst is not None, select_tcpip, select_usb]) <= 1
    )
    ...
    if (
        record_filename
        and record_format == symmetrical_doodle.options.RecordFormat.AUTO
    ):
